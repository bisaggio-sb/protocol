...
# Obszar "Wyniki turnieju" = szerokość lewej kolumny tabeli wynikowej.
# Nowy szablon IND zwęził tę kolumnę: 2970 dxa (5.24 cm) → 2694 dxa (4.75 cm).
# Centrowanie grafik MUSI się zgadzać z realną kolumną, inaczej grafiki wystają
# w tabelę. Clamp w generate_docx liczy szerokość kolumny dynamicznie z szablonu.
LEFT_AREA_CM = 4.75    # Indywidualny: szerokość lewej kolumny tabeli wynikowej (R1.tc[0] = 2694 dxa)
...
def compute_default_positions(active_keys, logos_aspect=None,
                              area_height_cm=18.0, area_width_cm=LEFT_AREA_CM,
                              template_type='IND'):
    ...
    # CZWORKA: strip pod tabelą — JEDNA pozioma linia: QR (po lewej) + grafiki.
    # QR.left  = 0    (lewa krawędź tabeli)
    # last.right = TABLE_RIGHT (prawa krawędź tabeli, 17.46 cm = TARGET_WIDTH 10466 dxa
    # minus mały zapas na cellMargin — krawędź "wizualna" widoczna w wydruku)
    # Pomiędzy: równomierne odstępy (gap = (span - sum_widths)/(n-1))
    if is_czworka:
        Y_QR       = 19.7        # offset Y w cm od kotwicy w R1.tc[0]
        Y_GRAPHICS = Y_QR        # rząd grafik wyrównany topem z QR
        QR_SIZE    = 1.6
        ROW_H      = 1.4         # docelowa wysokość rzędu grafik
        TABLE_RIGHT = 18.46      # prawa krawędź tabeli (= TARGET_WIDTH 10466 dxa / 567)

        # Lista elementów w kolejności od lewej: QR, PFM, logo1..4 (tylko aktywne)
        elements = []  # [(key, width_cm, height_cm)]
        if 'qr' in active_keys:
            elements.append(('qr', QR_SIZE, QR_SIZE))
        if 'pfm' in active_keys:
            elements.append(('pfm', ROW_H * pfm_aspect, ROW_H))
        for i in (1, 2, 3, 4):
            key = f'logo{i}'
            if key in active_keys:
                aspect = (logos_aspect or {}).get(key, 1.5)
                elements.append((key, ROW_H * aspect, ROW_H))
        ...
        total_w = sum(w for _, w, _ in elements)
        gap = (TABLE_RIGHT - total_w) / (n - 1) if n > 1 else 0
        # Jeśli gap < 0 (grafik za dużo szerokich), proporcjonalnie skaluj
        if gap < 0:
            scale = TABLE_RIGHT / total_w
            elements = [(k, w * scale, h * scale) for k, w, h in elements]
            total_w = sum(w for _, w, _ in elements)
            gap = 0

        cur_x = 0.0
        for key, w, h in elements:
            if key == 'qr':
                y = Y_QR
            else:
                # Wycentruj grafiki pionowo w slocie ROW_H (gdy są niższe niż ROW_H)
                y = Y_GRAPHICS + (ROW_H - h) / 2
            positions[key] = {'x': cur_x, 'y': y, 'w': w, 'h': h}
            cur_x += w + gap
        return positions
...
with col_form:
    ...
    if 'detected' in st.session_state and sheets_url.strip():
        ...
        if cached.get('sheet_id') == cur_sid:
            ...
            with st.expander("📊 Lista wszystkich faz w arkuszu (grupowanie po godzinie)", expanded=False):
                ...
                if total_matches > 0:
                    st.divider()
                    st.markdown(f"**Razem: {total_matches} {generate_docx.pluralize(total_matches,'mecz','mecze','meczów')}** w arkuszu "
    ...
    if is_no_graphics:
        ...
        # IND: bump |v2 — zwężenie lewej kolumny (5.24→4.75 cm) zmienia domyślne
        # pozycje grafik; resetuje stare zapisane wartości suwaków.
        _sig_tpl = {'CZWORKA': f'{_tpl_type}|cz5', 'IND': f'{_tpl_type}|v2'}.get(_tpl_type, _tpl_type)
...
with col_preview:
    ...
    elements_html = ""
    # CZWORKA: strip pod tabelą — QR i grafiki w JEDNEJ poziomej linii.
    # QR przy lewej krawędzi tabeli, ostatnia grafika przy prawej, równe odstępy.
    # Tekst "Wyniki" osobno, w połowie drogi między dolną krawędzią tabeli a rzędem icon.
    czworka_strip_items = []  # [(key, inline_html)] w kolejności od lewej

    # Mapping wysokości w preview (px) bazując na pos['h'] (cm) — żeby QR
    # i grafiki miały proporcjonalnie rozmiary jak w docx.
    for key in image_urls:
        ...
        if key == 'qr':
            ...
            # CZWORKA inline: QR z proporcjami z pos (1.6 cm × 16 = 26 px)
            inline_div = f"""
            <div style="width:{w_px}px; height:{h_px}px;
            ...
            inline_div = f"""
            <img src="{image_urls[key]}"
                 style="width:{w_px}px; height:{h_px}px; object-fit:contain;
                        border:1px dashed #ccc; flex-shrink:0;"/>"""
        if is_czworka:
            czworka_strip_items.append((key, inline_div))
        else:
            elements_html += absolute_div

    # Sortuj wg. pos.x — żeby kolejność w preview pasowała do layoutu docx
    czworka_strip_items.sort(key=lambda kv: image_positions[kv[0]]['x'])
    czworka_icons_html = "".join(html for _, html in czworka_strip_items)

    # Pozycja QR w preview pixels (dla "Wyniki" — centrowane nad QR z dopasowaną szerokością)
    if 'qr' in image_positions:
        _qrp = image_positions['qr']
        czworka_qr_x_px = _qrp['x'] * SCALE
        czworka_qr_w_px = _qrp['w'] * SCALE
    else:
        czworka_qr_x_px = 0
        czworka_qr_w_px = 26
    ...
    if is_trojka or is_czworka:
        ...
        <div style="background:white; border:1px solid #ccc;
          ...
          <!-- CZWÓRKA: strip pod tabelą.
               "Wyniki" wycentrowane w boxie o szerokości QR (nad QR).
               Pod nim rząd: QR + grafiki w jednej linii, justify-content:space-between. -->
          <div style="position:absolute; left:0; right:0; bottom:6px; height:64px;
                      border-top:1px dashed #ccc;
                      background:rgba(245,247,250,0.8);">
            <div style="position:absolute; left:{int(czworka_qr_x_px)}px;
                        width:{int(czworka_qr_w_px)}px; top:4px;
                        font-size:9px; font-weight:bold; color:#333;
                        text-align:center;">
              Wyniki
            </div>
            <div style="display:flex; justify-content:space-between;
                        align-items:flex-start; padding:22px 1px 0;">
              {czworka_icons_html}
            </div>
          </div>
          ''' if is_czworka else ''}
    ...
    if elements_active:
        ...
        # użytecznej strony (18.46 cm = TARGET_WIDTH) i niżej (y do ~25 cm).
        # SAFE_MARGIN: dla CZWORKA = 0 (grafiki mogą dotykać prawej krawędzi tabeli).
        # Dla IND/TROJKA = 0.1 cm żeby obraz nie dotykał krawędzi kolumny.
        if is_czworka:
            col_width = 18.46
            max_y_abs = 25.0
            SAFE_MARGIN = 0.0
        ...
        if is_czworka and elements_active:
            with st.expander("🔧 Debug pozycji (dla diagnostyki)", expanded=False):
                ...
                for k, _ in elements_active:
                    if k in image_positions:
                        p = image_positions[k]
                        r = p['x'] + p['w']
                        flag = "✓ touches right" if abs(r - _table_w) < 0.05 else (
                               "← past right!" if r > _table_w else f"({_table_w - r:.2f} cm short)")
                        rows.append(f"  {k}: x={p['x']:.2f} w={p['w']:.2f} → right={r:.2f}  {flag}")
                st.code("\n".join(rows))
...
# Karty 2 kolumny × kilka wierszy na A4 portrait. Każda karta ma imię,
# grupę, turniej/datę i tabelkę 4-kol (godzina | tor | gracz 1 | gracz 2).
# Idealne do wydruku, pocięcia i rozdania zawodnikom przy stoliku turniejowym.
# Dane derywowane z normalnych meczów grupowych (parse_group_rows) — każdy
# mecz ląduje w rozpiskach OBOJU graczy bez modyfikacji oryginalnej kolejności
# z arkusza.
rozp_clicked = False
rozp_fmt = 'PDF'
...
if gen_clicked:
    ...
    if selected_phase_keys_multi is not None and len(selected_phase_keys_multi) >= 1:
        if not is_supported_type:
            st.error(f"**{tournament_type} — faza pucharowa** nie jest obsługiwana. "
                     "Dostępne: Indywidualny (wszystkie fazy), Drużynowy 3-os. (wszystkie fazy), "
                     "Drużynowy 4-os. (tylko grupowa).")
            st.stop()
    ...
    if not _multi_phase_done:
        ...
        if is_pucharowa:
            ...
            def _on_fetch(done, tot, label):
                try:
                    # Fallback mode: label = "znaleziono N" → "Skanuję… (znaleziono N)"
                    # Normal mode: label = sheet name → "Pobieram Gr. X (3/5)…"
                    if label.startswith("znaleziono "):
                        msg = f"Skanuję arkusz… ({label})"
                    else:
                        msg = f"Pobieram {label} ({done}/{tot})…"
                    _fetch_pb.progress(min(1.0, done / max(1, tot)), text=msg)
                except Exception:
                    pass
            try:
                try:
                    sheets_data = generate_docx.fetch_all_group_sheets(sid, progress_cb=_on_fetch)
                except Exception as e:
        ...
        try:
            ...
            # IND pucharowa: Bo3 → IND_Bo3.docx, Bo5 → IND_Bo5.docx
            # Trójka pucharowa: Bo3 → TROJKA_Bo3.docx, Bo5 → TROJKA_Bo5.docx
            # Czwórka — tylko grupowa
            if is_individual and is_pucharowa and sets_format == "Best of 3":
                template_type = 'IND_Bo3'
        ...
        st.session_state['last_gen'] = {
            ...
            'name': safe_name,
            'total': total,
            'groups': locals().get('_orig_groups_count', len(sheets_data)),
            'kind': 'full',
...
if rozp_clicked:
    ...
    sid = extract_id(sheets_url.strip())
    if not sid:
        st.error("Nie udało się rozpoznać ID arkusza w linku."); st.stop()
    progress_box = st.empty()
    progress_box.info("Pobieram zakładki Gr. *…")
    ...
    safe_name = f"Rozpiski_{tournament_name.strip() or 'turniej'}".replace(' ', '_')
    safe_name = re.sub(r'[^A-Za-z0-9_\-]', '', safe_name) or 'Rozpiski'

    if rozp_fmt == 'PDF':
        progress_box.info("Konwertuję do PDF…")
        est_pages = max(1, (len(schedules) + 7) // 8)
        pdf_bytes, pdf_err = docx_to_pdf(docx_bytes, safe_name, est_pages=est_pages)
        progress_box.empty()
        if pdf_err or not pdf_bytes:
            st.error(f"Błąd konwersji do PDF: {pdf_err}")
            st.download_button(
                "⬇️ Pobierz rozpiski (DOCX zamiast PDF)",
